    ...
    >
      {uniqueItems?.map((item, i) => (
        <div
          ...
          key={item?.card?.info?.id || item?.info?.id || i}
          ...
          <div className="w-9/12">
            <div className="py-2">
              ...
              <span className="font-semibold">
                {" "}
                - ₹
                {(item?.card?.info?.price || item.info?.price) / 100 ||
                  (item?.card?.info?.defaultPrice || item.info?.defaultPrice) /
                    100}
              </span>
            </div>
            <p className="text-xs">
              {item?.card?.info?.description || item?.info?.description}
            </p>
          </div>
          <div className="w-full md:w-3/12 p-4">
            <div className="absolute">
              ...
              {cartItems?.find(
                (cart) =>
                  cart?.card?.info?.id ===
                  (item?.card?.info?.id || item?.info?.id)
              ) ? (
                location.pathname === "/cart" ? (
                  <button
                    className="p-2 mx-10 my-16 w-[100px] bg-red-500 text-white shadow-lg rounded-lg  object-cover border border-solid hover:bg-red-300"
                  ...
                  <button
                    className="p-2 mx-10 my-16 w-[100px] bg-slate-600 text-white shadow-lg rounded-lg  object-cover border border-solid hover:bg-slate-400"
                    onClick={() => goToCart()}
                  >
                    Go To Cart
                  </button>
                ...
                <button
                  className="p-2 mx-10 my-16 w-[100px] bg-white text-green-500 shadow-lg rounded-lg  object-cover border border-solid hover:bg-green-200"
                  onClick={() => handleAddButton(item)}
                >
                  Add +
                </button>
            ...
            {item?.card?.info?.imageId || item?.info?.imageId ? (
              <img
                src={`${MENU_API_CDN_IMG}${
                  item?.card?.info?.imageId || item?.info?.imageId
                }`}
